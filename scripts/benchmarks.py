...
def kept_bins(specs_shape, interp_steps):
...
def time_all_pipeline(num_iter):
    ...
    print(f"SLS-I (default): {average_time_ctfr:0.3f} s -- {100*average_time_ctfr/duration:0.2f}% real-time -- {100*kept_bins(specs.shape, interp_steps_default):.2f}% SLS -- {log_spectral_distortion(cspec_ctfr, cspec_base):.2f} LSD -- {relative_power_error(cspec_ctfr, cspec_base):e}")
    cspec_ctfr, average_time_ctfr = benchmark_method(specs, method='sls_i', num_iter=num_iter, interp_steps=interp_steps_double)
    print(f"SLS-I (doubled): {average_time_ctfr:0.3f} s -- {100*average_time_ctfr/duration:0.2f}% real-time -- {100*kept_bins(specs.shape, interp_steps_double):.2f}% SLS -- {log_spectral_distortion(cspec_ctfr, cspec_base):.2f} LSD -- {relative_power_error(cspec_ctfr, cspec_base):e}")
